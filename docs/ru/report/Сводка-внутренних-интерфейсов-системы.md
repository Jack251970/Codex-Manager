...
| Вход | `service_login_cancel` | `account/login/cancel` | Параметр: `loginId`; отменяет выполняющийся процесс входа |
...
| Gateway | `service_gateway_transport_set` | `gateway/transport/set` | Параметры: `sseKeepaliveEnabled`, `sseKeepaliveIntervalMs`, `upstreamTotalTimeoutMs`, `upstreamStreamTimeoutMs` |
...
| Codex Skills | `service_codex_skills_list` | `codexSkills/list` | Необязательный параметр: `codexHome`; сканирует пользовательские и встроенные Skills |
| Codex Skills | `service_codex_skills_install_zip` | `codexSkills/installZip` | Параметры: `fileName`, `archiveBase64`; необязательный: `codexHome` |
| Codex Skills | `service_codex_skills_import_directory` | `codexSkills/importDirectory` | Параметр: `sourcePath`; необязательный: `codexHome` |
| Codex Skills | `service_codex_skills_delete` | `codexSkills/delete` | Параметр: `directoryName`; необязательный: `codexHome` |
| Codex Skills | `service_codex_skills_marketplace_list` | `codexSkills/marketplaceList` | Необязательный параметр: `codexHome`; выводит Marketplace и доступные плагины |
| Codex Skills | `service_codex_skills_marketplace_add` | `codexSkills/marketplaceAdd` | Параметр: `source`; необязательные: `refName`, `codexHome` |
| Codex Skills | `service_codex_skills_marketplace_refresh` | `codexSkills/marketplaceRefresh` | Необязательные параметры: `marketplaceName`, `codexHome` |
| Codex Skills | `service_codex_skills_marketplace_plugin_install` | `codexSkills/marketplacePluginInstall` | Параметр: `pluginId`; необязательный: `codexHome`; устанавливает полный плагин Codex |
