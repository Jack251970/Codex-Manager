...
| 登录 | `service_login_cancel` | `account/login/cancel` | 参数：`loginId`；取消进行中的登录流程 |
...
| 网关 | `service_gateway_transport_set` | `gateway/transport/set` | 参数：`sseKeepaliveEnabled`、`sseKeepaliveIntervalMs`、`upstreamTotalTimeoutMs`、`upstreamStreamTimeoutMs` |
...
| Codex Skills | `service_codex_skills_list` | `codexSkills/list` | 参数可选：`codexHome`；扫描用户与内置 Skills |
| Codex Skills | `service_codex_skills_install_zip` | `codexSkills/installZip` | 参数：`fileName`、`archiveBase64`；可选：`codexHome` |
| Codex Skills | `service_codex_skills_import_directory` | `codexSkills/importDirectory` | 参数：`sourcePath`；可选：`codexHome` |
| Codex Skills | `service_codex_skills_delete` | `codexSkills/delete` | 参数：`directoryName`；可选：`codexHome` |
| Codex Skills | `service_codex_skills_marketplace_list` | `codexSkills/marketplaceList` | 参数可选：`codexHome`；列出 Marketplace 与可安装插件 |
| Codex Skills | `service_codex_skills_marketplace_add` | `codexSkills/marketplaceAdd` | 参数：`source`；可选：`refName`、`codexHome` |
| Codex Skills | `service_codex_skills_marketplace_refresh` | `codexSkills/marketplaceRefresh` | 参数可选：`marketplaceName`、`codexHome` |
| Codex Skills | `service_codex_skills_marketplace_plugin_install` | `codexSkills/marketplacePluginInstall` | 参数：`pluginId`；可选：`codexHome`；安装完整 Codex 插件 |
