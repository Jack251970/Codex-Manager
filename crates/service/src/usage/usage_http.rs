...
fn reset_credit_request_headers(workspace_id: Option<&str>) -> HeaderMap {
...
async fn fetch_reset_credits_snapshot_async(
    ...
    let build_request = || {
        usage_http_client()
            ...
            .headers(reset_credit_request_headers(workspace_id))
    };
    let response = build_request()
        ...
        .map_err(|error| UsageActionHttpError {
            status: None,
            message: format!("request reset credits failed: {error}"),
        })?;
...
async fn consume_reset_credit_request_async(
    ...
    let response = usage_http_client()
        .post(&url)
        .header("Authorization", format!("Bearer {bearer}"))
        .headers(reset_credit_request_headers(workspace_id))
